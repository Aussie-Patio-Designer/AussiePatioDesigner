...
interface Agent {
  ...
  email: string
  phone?: string
  website?: string // This matches the database column name
  logo_url?: string
  url_slug: string
  status: string
  ...
  const fetchAgents = async () => {
    try {
      ...
      if (response.ok && data.success) {
        ...
        setAgentsError(data.error || "Failed to fetch agents")
        console.error("Error response:", data)
      }
    } catch (error) {
      console.error("Error fetching agents:", error)
      setAgentsError("Network error while fetching agents")
    } finally {
  ...
  const handleAddAgent = async (agentData: any) => {
    try {
      ...
      if (response.ok && result.success) {
        ...
        const customerUrl = result.agent.customer_url
        ...
        alert(`✅ Agent created successfully!\n\nCustomer URL: ${customerUrl}${clipboardMessage}`)
      } else {
      ...
      alert("❌ Network error: Failed to create agent. Please check your connection and try again.")
    }
  }

  const handleEditAgent = async (agentData: any) => {
    try {
      const response = await fetch(`/api/admin/agents/${agentData.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(agentData),
      })
      ...
      alert("❌ Network error: Failed to update agent. Please check your connection and try again.")
    }
  }

  const handleDeleteAgent = async (agent: Agent) => {
    const confirmMessage = `⚠️ DELETE AGENT: ${agent.company_name}

This will permanently delete:
• Agent account and settings
• All inquiries sent to this agent (${agent.email})
• Cannot be undone
...
Type "DELETE" to confirm:`
    ...
    try {
      ...
      if (response.ok && result.success) {
        await fetchAgents()
        alert(
          `✅ Agent "${agent.company_name}" and all related data deleted successfully!\n\nDeleted ${result.details?.inquiries_deleted || 0} inquiries.`,
        )
      } else {
        const errorMessage = result.error || "Unknown error occurred"
        alert(`❌ Error: ${errorMessage}`)
      ...
      alert("❌ Network error: Failed to delete agent. Please check your connection and try again.")
    }
  }
  ...
  const showUrlDialog = (url: string) => {
    // Show the URL in a dialog for manual copying
    const message = `Please copy this URL manually:\n\n${url}\n\nClick OK to close this dialog.`
    alert(message)
  }
              ...
              <CardContent>
                {agentsLoading ? (
                  ...
                  <div className="text-center py-8">
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4 inline-block">
                      <p className="text-amber-800 font-medium">⚠️ {agentsError}</p>
                      <p className="text-amber-700 text-sm mt-1">
                        The agents table will be created automatically when you add your first agent.
                      </p>
                    </div>
                    <div>
                      <Button onClick={() => setShowAddAgent(true)} className="bg-green-600 hover:bg-green-700">
                          ...
                          <tr key={agent.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="p-3">
                              <div className="flex items-center gap-2">
                                <a
                                  href={`https://aussie-patio-designer.vercel.app/${agent.url_slug}`}
                                  target="_blank"
                                  rel="noopener noreferrer"
                            ...
                            <td className="p-3">
                              <div className="flex gap-2">
                                <Button variant="outline" size="sm" onClick={() => openEditModal(agent)}>
                                  ✏️ Edit
                                </Button>
                                <Button
                                  variant="outline"
