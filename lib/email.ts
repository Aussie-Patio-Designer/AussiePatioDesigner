...
import { isValidEmail } from "./email-verification"
...
async function sendSalesTeamNotification(
  ...
  // CRITICAL: Verify the email is valid before attempting to send
  if (!isValidEmail(recipientEmail)) {
    console.error(`❌ INVALID EMAIL ADDRESS: "${recipientEmail}". Falling back to default.`)

    // Fall back to the default email
    const fallbackEmail = process.env.SALES_EMAIL_1 || "gaziogutcu.go@gmail.com"

    console.log(`⚠️ Falling back to default email: ${fallbackEmail}`)

    // If fallback is also invalid, we can't send
    if (!isValidEmail(fallbackEmail)) {
      console.error(`❌ FALLBACK EMAIL ALSO INVALID: "${fallbackEmail}". Cannot send notification.`)
      return {
        sent: false,
        error: "Invalid email address and invalid fallback. Cannot send notification.",
      }
    }

    // Use fallback email instead
    const recipientEmail = fallbackEmail
  }
  ...
  const subject = isAgentInquiry
    ? `New Patio/Gazebo Inquiry ${referenceNumber} - ${data.customerName} (via ${companyName})`
    : `New Patio/Gazebo Inquiry ${referenceNumber} - ${data.customerName}`

  // Update the email header to show agent info
  const emailHeader = isAgentInquiry
    ? `
      <div class="alert-badge">AGENT INQUIRY</div>
      <h1>Patio/Gazebo Design Request</h1>
      <div class="reference">Reference: ${referenceNumber}</div>
      <div class="reference">Agent: ${companyName}</div>
...
Generated by Gazi OGUTCU 2025
  ...
  try {
    ...
    // CRITICAL: Log the exact request we're sending to Resend
    ...
    const result = await resend.emails.send({
      from: "Aussie Patio Inquiries <onboarding@resend.dev>",
      to: [recipientEmail], // Single email address in an array
      subject: subject,
      text: textContent,
      html: htmlContent,
    })
    ...
    // Try to send to fallback if agent email failed
    if (isAgentInquiry && process.env.SALES_EMAIL_1) {
      console.log("⚠️ Agent email failed, trying fallback email:", process.env.SALES_EMAIL_1)

      try {
        const resend = new Resend(config.RESEND_API_KEY)
        const fallbackResult = await resend.emails.send({
          from: "Aussie Patio Inquiries <onboarding@resend.dev>",
          to: [process.env.SALES_EMAIL_1],
          subject: `[FALLBACK] ${subject}`,
          text: `ORIGINAL RECIPIENT: ${recipientEmail}\n\n${textContent}`,
          html: `<div style="background:#ffebee;padding:15px;margin-bottom:20px;border:1px solid #ffcdd2;border-radius:4px">
                  <p style="color:#c62828;font-weight:bold">⚠️ FALLBACK DELIVERY</p>
                  <p>Original recipient (${recipientEmail}) failed. This is a fallback delivery.</p>
                </div>${htmlContent}`,
        })
        ...
        return {
          sent: true,
          recipients: [process.env.SALES_EMAIL_1],
          error: `Original recipient failed: ${error instanceof Error ? error.message : "Unknown error"}`,
        }
      } catch (fallbackError) {
  ...
  try {
    ...
    if (data.agentInfo && inquiryId) {
      try {
        ...
        console.log("🎯 AGENT INFO SAVED TO DATABASE:", {
          inquiryId,
          agentEmail: data.agentInfo.email,
          agentCompany: data.agentInfo.company_name,
    ...
    // 🚨 CRITICAL LOGGING FOR TRACKING
    console.log("🎯 FINAL EMAIL ROUTING SUMMARY:")
    console.log("📧 Customer email sent:", customerEmailResult.sent)
    console.log("📧 Sales email sent:", salesEmailResult.sent)
    console.log("📧 Sales email recipients:", salesEmailResult.recipients || "Unknown")
    console.log("🏢 Agent company:", data.agentInfo?.company_name || "None")
    console.log("📍 Source URL:", data.sourceUrl || "Unknown")
    ...
    return {
      ...
      emailRecipients: salesEmailResult.recipients,
    }
  } catch (error) {
