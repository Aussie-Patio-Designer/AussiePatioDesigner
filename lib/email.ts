...
// Centralized sender address – override via EMAIL_FROM env var.
// Falls back to Resend onboarding domain when no verified domain is configured.
function getSenderAddress(): string {
  return process.env.EMAIL_FROM || "Aussie Patio Designer <onboarding@resend.dev>"
}
...
async function sendCustomerConfirmation(
  ...
  // Prepare screenshot attachment if upload failed but we still have base64 data
  let attachments: {
    filename: string
    content: Buffer
    contentType: string
  ...
  if (!screenshotUrl && data.screenshot) {
    try {
      ...
      console.error("Failed to process screenshot attachment:", err)
    }
  }
                    ...
                    <td colspan="2" style="padding: 20px; text-align: center; border-bottom: 1px solid #e0e0e0;">
                      ${
                        screenshotUrl
                          ...
                          : data.screenshot
                          ? `<img src="cid:design-screenshot" alt="3D Patio/Gazebo Design" style="max-width: 100%; height: auto; border: 1px solid #e0e0e0; border-radius: 4px;" />`
...
async function sendSalesTeamNotification(
  ...
  // Prepare screenshot attachment if upload failed but base64 data is available
  let attachments: {
    filename: string
    content: Buffer
    contentType: string
                    ...
                    <td colspan="2" style="padding: 20px; text-align: center; border-bottom: 1px solid #e0e0e0;">
                      ${
                        screenshotUrl
                          ...
                          : data.screenshot
                          ? `<img src="cid:design-screenshot" alt="3D Patio/Gazebo Design Preview" style="max-width: 100%; height: auto; border: 1px solid #e0e0e0; border-radius: 4px;" />`
...
3D DESIGN:
${screenshotUrl ? `Screenshot: ${screenshotUrl}` : data.screenshot ? "Screenshot attached" : "Screenshot not available"}
